  ...
  context "instance" do
    let(:row) do
      CSV.parse(<<~CSV, headers: true, col_sep: ";").first
        ...
        00.00;Title;http://example.com;3.1.0;Nov 18 1994 12:00AM;1;Rapeli;Juha;Org;SP;R6, CP;UMTS;;;SA#47;SA#49;SA#51;SA#65;Aug 18 1994 12:00AM;Feb 12 1999 12:00AM
      CSV
    end
    ...
    it "parse doc" do
      ...
      expect(subject).to receive(:number).and_return(:number)
      ...
      expect(subject).to receive(:parse_radiotechnology).and_return(:radiotechnology)
      expect(subject).to receive(:parse_release).and_return(:release)
      expect(subject).to receive(:parse_contributor).and_return(:contributor)
      expect(Relaton3gpp::BibliographicItem).to receive(:new).with(
        type: "standard", language: ["en"], script: ["Latn"], title: :title,
        link: :link, docid: :docid, docnumber: :number, date: :date, doctype: :doctype,
        editorialgroup: :editorialgroup, radiotechnology: :radiotechnology, release: :release,
        contributor: :contributor, place: ["Sophia Antipolis Cedex, France"],
      ).and_return(:bibitem)
    ...
    it "parse docid" do
      ...
      expect(docid.first.id).to eq "3GPP TS 00.00:UMTS/3.1.0"
