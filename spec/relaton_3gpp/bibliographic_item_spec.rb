RSpec.describe Relaton3gpp::BibliographicItem do
  ...
  it "warn if invalid radiotechnology type" do
    ...
    end.to output(/WARNING: Unknown radiotechnology type: `invalid`/).to_stderr
  end

  it "warn if invalid doctype" do
    expect do
      Relaton3gpp::BibliographicItem.new(doctype: "invalid")
    end.to output(/WARNING: Unknown doctype: `invalid`/).to_stderr
  end

  it "warn if doctype is missing" do
    expect do
      Relaton3gpp::BibliographicItem.new
    end.to output(/WARNING: Doctype is missing/).to_stderr
  end

  it "warn if invalid docsubtype" do
    expect do
      Relaton3gpp::BibliographicItem.new(docsubtype: "invalid")
    end.to output(/WARNING: Unknown docsubtype: `invalid`/).to_stderr
  end
end
